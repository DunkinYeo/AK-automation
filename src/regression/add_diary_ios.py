"""
TC-DIARY (iOS): Log Symptoms sheet regression tests.
Mirrors add_diary.py but imports close_sheet / go_to_main from helpers_ios.
"""
import time
import random
import logging

from src.regression.helpers_ios import close_sheet, go_to_main
...
def test_diary_005_save_without_symptom(drv, runner):
    ...
    if enabled:
        log.warning("TC-DIARY-005 (iOS): Save enabled without symptom (server-side validation)")
        drv.tap_text("Save", timeout=5)
        time.sleep(1.5)
        if drv.is_visible_text("Symptom", timeout=2):
            _close_sheet(drv)
    else:
        log.info("TC-DIARY-005 (iOS): Save disabled without symptom")
        _close_sheet(drv)
