...
powershell -NoProfile -ExecutionPolicy Bypass -Command "Unblock-File -Path '%_PTZIP%' -ErrorAction SilentlyContinue; Expand-Archive -Path '%_PTZIP%' -DestinationPath '%CD%\runtime' -Force; Get-ChildItem '%CD%\runtime' -Recurse | Unblock-File -ErrorAction SilentlyContinue"
del "%_PTZIP%" >nul 2>&1
