#!/usr/bin/env bash
# ============================================================
# AccurKardia -- Launch Test Environment
...
LOG_FILE="/tmp/ak_run_$(date +%Y%m%d_%H%M%S).log"
echo "AccurKardia run started $(date)" >> "$LOG_FILE"
...
if [ ! -f "web/app.py" ]; then
    ...
    echo "  Run run.command from inside the AccurKardia-Automation folder."
    echo ""
...
if curl -sf --max-time 2 "http://127.0.0.1:4723/status" >/dev/null 2>&1; then
    ...
    if ! command -v appium >/dev/null 2>&1; then
        ...
        echo "[run] Starting Appium PID=$APPIUM_PID" >> "$LOG_FILE"
    fi
fi
...
# ── Sleep prevention (tied to web server, survives terminal close) ─
CAFF_PID=""
if command -v caffeinate >/dev/null 2>&1; then
    caffeinate -dims -w "$WEB_PID" &
    CAFF_PID=$!
